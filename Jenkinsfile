pipeline {
    ...
    environment {
        SERVICE_NAME = "ts-client-web-bff"
        HTTP_PORT = 8080
    ...
    stages {
        ...
        stage("Deploy"){
            steps {
                ...
                sh "sudo docker run --detach --name ${SERVICE_NAME} -p 8021:${HTTP_PORT} -e HTTP_PORT=${HTTP_PORT} -e DB_URL=${DB_URL} -e DB_USERNAME=${DB_USERNAME} -e DB_PASSWORD=${DB_PASSWORD} --network=service_postgresql--service --network=service_nginx--ts-client-web-bff jetlabs/${SERVICE_NAME}:latest"
            }
        }
    }
}
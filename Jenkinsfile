    ...
    stages {
        ...
        stage("Deploy"){
            steps {
                ...
                sh "sudo docker run --detach --name ${SERVICE_NAME} -e HTTP_PORT=${HTTP_PORT} --network=service_postgresql--service --network=service_nginx--ts-client-web-bff jetlabs/${SERVICE_NAME}:latest"
            }
        }
    }
}